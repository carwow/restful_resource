...
def expect_patch(url, response, data: {})
  expect(@mock_http).to receive(:patch).with(url, data: data).and_return(response)
end
...
def expect_patch_with_unprocessable_entity(url, response, data: {})
  rest_client_response = OpenStruct.new({body: response.body, headers: response.headers, code: response.status})
  exception = RestfulResource::HttpClient::UnprocessableEntity.new(rest_client_response)
  expect(@mock_http).to receive(:patch).with(url, data: data).and_raise(exception)
end
